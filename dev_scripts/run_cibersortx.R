...
blood_ref %>%
  ...
  write_tsv("../xCell2.0/CIBERSORTx_docker/cibersortx_blood_ref.txt")
...
pheno.df <- t(sapply(all_celltypes, function(ct){
  dep_vec <- sapply(blood_labels$label, function(label){
    ifelse(ct == label, 1,
...
pheno.df %>%
  as.data.frame(.) %>%
  rownames_to_column() %>%
  write_tsv("../xCell2.0/CIBERSORTx_docker/cibersortx_blood_pheno_df.txt", col_names = FALSE)

# sig_mat_blood <- "/Users/almogang/Documents/xCell2.0/CIBERSORTx_docker/CIBERSORTx_cibersortx_blood_pheno_df.CIBERSORTx_cibersortx_blood_ref.bm.K999.txt"
