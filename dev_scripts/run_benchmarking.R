...
xcell2_blood_refsigs <- readRDS("../../xCell2.0/xcell2_blood_refsigs_16_2_23_500genes_gubbs05.rds")
xcell2_tumor_refsigs <- readRDS("../../xCell2.0/xcell2_tumor_refsigs_16_2_23_500genes_gubbs05.rds")
xcell2_bp_refsigs <- readRDS("../../xCell2.0/xcell2_bp_refsigs_16_2_23_500genes_gubbs05.rds")
...
getCorrelations <- function(dataset, celltypes2add = c("Monocytes", "Neutrophils", "NK cells", "cDC", "T-cells", "Tregs",
                                                       ...
                                                       "Fibroblasts", "Lymphocytes", "Cancer cells", "CD8+ T-cells PD1 low",
						"CD8+ T-cells PD1 high", "Macrophages", "Macrophages M1", "Macrophages 
M2", "Endothelial cells", "Non plasma B-cells", "Follicular T-helper", "Myeloid", "Th1 cells", "Th2 cells",
 "Tgd cells", "Th17 cells")){
  ...
  epic_bref.out <- read.table(paste0("../../xCell2.0/Kassandara_data/predicted_by_algorithms/EPIC_BRef/", dataset, "_predicted_by_EPIC_BRef.tsv"), header = TRUE, check.names = FALSE, sep = "\t", row.names 
= 
1)
  ...
  epic_tref.out <- read.table(paste0("../../xCell2.0/Kassandara_data/predicted_by_algorithms/EPIC_TRef/", dataset, "_predicted_by_EPIC_TRef.tsv"), header = TRUE, check.names = FALSE, sep = "\t", row.names 
= 
1)
  ...
  cbrx_hnsc.out <- read.table(paste0("../../xCell2.0/Kassandara_data/predicted_by_algorithms/HNSC_matrix_with_BC_CIBERSORTX/", dataset, "_predicted_by_HNSC_matrix_with_BC_CIBERSORTX.tsv"), header = TRUE, 
check.names = FALSE, sep = "\t", row.names = 1)
  ...
  cbrx_lm22.out <- read.table(paste0("../../xCell2.0/Kassandara_data/predicted_by_algorithms/LM22_matrix_with_BC_CIBERSORTX/", dataset, "_predicted_by_LM22_matrix_with_BC_CIBERSORTX.tsv"), header = TRUE, 
check.names = FALSE, sep = "\t", row.names = 1)
  ...
  quanti.out <- read.table(paste0("../../xCell2.0/Kassandara_data/predicted_by_algorithms/quantiseq/", dataset, "_predicted_by_quantiseq.tsv"), header = TRUE, check.names = FALSE, sep = "\t", row.names = 
1)
  ...
  quanti_tumor.out <- read.table(paste0("../../xCell2.0/Kassandara_data/predicted_by_algorithms/quantiseq_tumor/", dataset, "_predicted_by_quantiseq_tumor.tsv"), header = TRUE, check.names = FALSE, sep = 
"\t", row.names = 1)
