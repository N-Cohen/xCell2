...
filterSignatures <- function(signatures_ranked, ranks_weights = c(.5, .25, .25), take_top_per = .1){

  signatures_filtered <- signatures_ranked %>%
    mutate(CT_rank = coalesce(CT_rank, CTOI_rank), diff_rank = coalesce(diff_rank, CTOI_rank)) %>%
    mutate(final_rank = ranks_weights[1]*CTOI_rank + ranks_weights[2]*CT_rank + ranks_weights[3]*diff_rank) %>%
